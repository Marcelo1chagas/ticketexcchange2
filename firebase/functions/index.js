...
exports.onUserDeleted = functions
  ...
  .onDelete(async (user) => {
    ...
    await firestore
      .collectionGroup("leiao")
      .where("usersREF_donoingresso", "==", userRef)
      .get()
      .then(async (querySnapshot) => {
        for (var doc of querySnapshot.docs) {
          console.log(`Deleting document ${doc.id} from collection leiao`);
          await doc.ref.delete();
        }
      });
    await firestore
      ...
      .then(async (querySnapshot) => {
        for (var doc of querySnapshot.docs) {
          console.log(`Deleting document ${doc.id} from collection ingressos`);
          await doc.ref.delete();
        }
      });
    await firestore
    ...
    await firestore
      .collection("compra-AUMENTAR-ALCANCE-15dias")
      .where("usersREF", "==", userRef)
      .get()
      .then(async (querySnapshot) => {
        for (var doc of querySnapshot.docs) {
          console.log(
            `Deleting document ${doc.id} from collection compra-AUMENTAR-ALCANCE-15dias`,
          );
          await doc.ref.delete();
        }
      });

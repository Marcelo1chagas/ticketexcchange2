...
class FirebaseAuthManager extends AuthManager
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Muito tempo desde o último login. Faça login antes de deletar a conta')),
        );
      }
    }
  }
    ...
    try {
      ...
      if (e.code == 'requires-recent-login') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Muito tempo desde o último login. Faça login antes de alterar o e-mail')),
        );
      }
    }
  }
  ...
  Future resetPassword({
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('E-mail para alterar senha enviado')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : TicketExchangeFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'Este e-mail já esta sendo utilizado',
        'INVALID_LOGIN_CREDENTIALS' => 'Credencial inválida ou expirada',
        _ => 'Erro',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
